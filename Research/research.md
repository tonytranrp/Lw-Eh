...
11. **Open decisions to resolve before writing code** (not answered by this research at the time it was written — these were judgment calls for the project owner). All three have since been resolved during implementation; recorded here rather than left stale, since this file is the standing technical reference future firings re-skim rather than work from memory of:
    - *Default max-listener-count model (compile-time NTTP array vs. intrusive list, or support both as policies)* → resolved as **both, as separate policies**: `lweh::signal<Event, MaxListeners>` (fixed-capacity array, `include/lweh/signal.hpp`) and `lweh::intrusive_signal<Event>` (singly-linked, listener-is-the-node, `include/lweh/intrusive_signal.hpp`) — see Research/ARCHITECTURE.md and the README's storage-policy comparison table.
    - *Whether v1 supports only synchronous dispatch or also an ISR-safe queued mode* → resolved as **synchronous-only, deliberately, not deferred**: both `signal.hpp` and `detail/intrusive_core.hpp` explicitly document "None of the above is safe against concurrent/ISR reentrancy... that needs the caller's own synchronization, deliberately not baked in here," citing this exact section and the Part A §6 Boost.Signals2 lesson ("never make thread-safety an unconditional default cost"). Not a gap to fill — an EnTT-style queued/ISR-safe dispatch mode is a real, heavier feature this project has chosen not to carry, matching its whole minimalism stance; revisit only if a concrete consumer need arises, not speculatively.
    - *Which exact C++ standard to formally pin* → resolved as **C++17**: `target_compile_features(lw_eh INTERFACE cxx_std_17)` in the root `CMakeLists.txt`.

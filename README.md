...
Measured incremental footprint on a real ESP32 (Xtensa) build: **888 bytes** for six event types split across both storage policies below — see [Research/PROGRESS.md](Research/PROGRESS.md) for the current number and how it's measured (this number moves as the reference scenario gains event types, so treat it as a snapshot, not a promise; most of its recent history — 471 → 564 → 822 → 824 — was measurement-methodology corrections, not Lw-Eh's own code growing, but the latest jump to 888 is real: the example now also exercises `detach()` on real hardware for both storage policies, not just attach/publish). Size aside, this isn't just a static measurement: the example has been flashed to and run on a real ESP32 board, with every core API surface and documented boundary condition — attach, publish, detach, and `signal<Event,N>` correctly rejecting an attach once full — exercised on both storage policies end-to-end, not merely cross-compiled and sized.

...
// Shared "realistic Lw-Eh usage" scenario -- six event types exercising
...
// free-function listener) and intrusive_signal<Event> (connection_event and
// motion_event: intrusive_node listeners) -- reused verbatim by every
// architecture's embedded entry point (esp32_minimal/, riscv32_esp_minimal/,
...
// original Phase-0 stub state (see Research/PROGRESS.md).
//
...
// PROGRESS.md).
//
// motion_event is a SECOND intrusive_signal<>-backed type (firing 34),
// mirroring connection_event's exact shape (a single bool field, one
// intrusive_node listener) for the same reason wifi_rssi_event mirrors
// battery_level_event: isolating "one more intrusive_signal<> instantiation"
// as the only variable. Before this, the scenario had exactly one
// intrusive_signal<>-backed type -- meaning intrusive_signal.hpp's own
// doc-comment claim ("genuinely shares one non-template compiled engine
// across every Event type") had never actually been exercised by more than
// one instantiation anywhere in this codebase, unlike signal<>'s equivalent
// claim, which firing 18's real disassembly-level verification put to the
// test directly (finding publish() folds across signal<> instantiations,
// attach() never does). This addition exists to make that same class of
// verification possible for intrusive_signal<> too.
...
inline lweh::intrusive_signal<motion_event> g_motion_signal;
...
class motion_logger : public lweh::intrusive_node<motion_event> {
public:
    void on_event(const motion_event& e) override {
        detected = e.detected;
    }
    bool detected = false;
};

inline motion_logger g_motion_logger;

// pin_input/sensor_input/connected_input/battery_input/wifi_input/
// motion_input should come from a volatile read at the call site (or be
// volatile themselves) when this is used for size measurement, so the
// aggressive flag set can't fully constant-fold the whole scenario away --
// confirmed necessary empirically (Research/PROGRESS.md's delegate-vs-signal
// host-proxy size-probe findings, firings 2-3). The embedded example entry
// point doesn't need this care since it's not being used to measure
...
inline void run_scenario(unsigned pin_input, int sensor_input, bool connected_input,
                          unsigned battery_input, int wifi_input, bool motion_input) {
    ...
    g_motion_signal.attach(g_motion_logger);
    ...
    g_motion_signal.publish(motion_event{motion_input});
}
